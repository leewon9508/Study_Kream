...
<body>
	<div id="header">
       <div class="name">포인트 사용 내역 + pointuselist</div>
        <div id=top>
	...
	<!-- 페이징 -->
		<table>
			...
			<c:forEach var="point" items="${pointaccountuselist}">
				<tr>
					<td>${point.dprice}</td>
					<td>${point.ddate}</td>
				</tr>
		...
		<c:choose>
			...
			<c:otherwise>
				<a href="pointchargelist?page=${paging.page-1}&dbidid=${sessionScope.loginMember}">[이전]</a>&nbsp;
			</c:otherwise>
		</c:choose>
		...
		<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="i"
			...
			<c:choose>
				...
				<c:otherwise>
					<a href="pointchargelist?page=${i}&dbidid=${sessionScope.loginMember}">${i}</a>
		...
		<c:choose>
			...
			<c:otherwise>
				<a href="pointchargelist?page=${paging.page+1}&dbidid=${sessionScope.loginMember}">[다음]</a>
			</c:otherwise>
		</c:choose>
	</div>

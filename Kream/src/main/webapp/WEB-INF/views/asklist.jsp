...
<script>
// 판매 입찰 취소
function askDelete (number, askid) {
	console.log(number);
	console.log(askid);
	var check = confirm('입찰을 취소 하시겠습니까?');
	if (check) {
		console.log(number);
		location.href = 'askdelete?asknumber=' + number + '&askid=' + askid;
		alert('최소 되었습니다.');
	} else {
		alert('삭제를 취소 합니다.');
	}
...
<body>
<div id="header">
        <div class="name">판매 입찰 리스트 + asklist</div>
        <div id=top>
		...
		<!-- 페이징 -->
			<table>
				...
				<c:forEach var="ask" items="${askList}">
					<tr>
						...
						<td><button onclick="askDelete('${ask.asknumber}','${sessionScope.loginMember}')">취소</button></td>
					</tr>
				</c:forEach>
			</table>
				...
				<c:otherwise>
					<a href="asklistpage?page=${paging.page-1}&askid=${sessionScope.loginMember}">[이전]</a>&nbsp;
				</c:otherwise>
			</c:choose>
			...
			<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="i"
				...
				<c:choose>
					...
					<c:otherwise>
						<a href="asklistpage?page=${i}&askid=${sessionScope.loginMember}">${i}</a>
			...
			<c:choose>
				...
				<c:otherwise>
					<a href="asklistpage?page=${paging.page+1}&askid=${sessionScope.loginMember}">[다음]</a>
				</c:otherwise>
			</c:choose>
	</div>

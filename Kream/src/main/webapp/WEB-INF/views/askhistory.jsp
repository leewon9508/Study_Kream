...
<body>
<div id="header">
        <div class="name">판매 내역 + askhistory</div>
        <div id=top>
...
<div id="nav">
	...
	<div id="content">
		<table>
			...
			<c:forEach var="askHistory" items="${askHistory}">
				<tr>
					<td>${askHistory.daskid}</td>
					<td><a
						href="productview?pnumber=${askHistory.dpnumber}&page=${paging.page}">${askHistory.dpnumber}</a></td>
					<td>${askHistory.dsize}</td>
					<td>${askHistory.dprice}</td>
					<td>${askHistory.ddate}</td>
					<td><a
						href="dealview?dnumber=${askHistory.dnumber}&pnumber=${askHistory.dpnumber}&id=${askHistory.daskid}">조회</a></td>
				</tr>
			</c:forEach>
		</table>
		<c:choose>
			...
			<c:otherwise>
				<a href="askhistory?page=${paging.page-1}&daskid=${sessionScope.loginMember}">[이전]</a>&nbsp;
		</c:otherwise>
		</c:choose>
		...
		<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="i"
			...
			<c:choose>
				...
				<c:otherwise>
					<a href="askhistory?page=${i}&daskid=${sessionScope.loginMember}">${i}</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:choose>
			...
			<c:otherwise>
				<a href="askhistory?page=${paging.page+1}&daskid=${sessionScope.loginMember}">[다음]</a>
			</c:otherwise>
		</c:choose>
	</div>
		<div id="footer">
